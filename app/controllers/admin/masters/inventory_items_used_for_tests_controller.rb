class Admin::Masters::InventoryItemsUsedForTestsController < ApplicationController
  layout 'admin'
  ...
  def create
    ...
    if @inventory_item_used_for_test.save
      ...
      redirect_to(admin_masters_lab_test_inventory_items_used_for_tests_path(@lab_test))
    else
      render :action => "new"
    end
  end
