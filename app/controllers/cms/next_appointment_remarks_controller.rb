class  Cms::NextAppointmentRemarksController < ApplicationController
  ...
  def create
    ...
    if @next_appointment_remark.save
      @new_appointment =  Appointment.create(:doctor_id => current_user.id, :reason_id => @appointment.reason_id, :patient_id => @appointment.patient_id, :appointment_date => params[:date], :hour => params[:next_appointment_remark][:hour], :minute => params[:next_appointment_remark][:minute], :mode_id => @appointment.mode_id, :created_by_id => current_user.id)
